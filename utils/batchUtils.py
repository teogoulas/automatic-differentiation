...
def generate_batches(y_batch):
    y_batch_circle = torch.tensor([1 if i in [1, 2, 4, 5, 7] else 0 for i in y_batch.tolist()], dtype=torch.float32)
    y_batch_curve = torch.tensor([1 if i in [1, 2, 4, 5, 7] else 0 for i in y_batch.tolist()], dtype=torch.float32)
